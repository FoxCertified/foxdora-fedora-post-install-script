...
##Remove if present, not needed.
sudo dnf -y remove kwrite
##Fedora
sudo dnf -y install flatseal flatpak-kcm btrfs-assistant gparted kate gnome-disk-utility
##Flatpak
...
##Extra Utility
echo ""
echo "Would you like to install OBS Studio(Screen Recorder),"
echo "EasyEffects Audio Filtering(Nvidia Broadcast Alternative, any hardware),"
echo "and a Youtube Video Downloader?"
set -- $(locale LC_MESSAGES)
yesexpr="$1"; noexpr="$2"; yesword="$3"; noword="$4"
while true; do
    read -p "(${yesword} / ${noword})? " yn
    if [[ "$yn" =~ $yesexpr ]]; then echo "Accepted";./scripts/subscripts/extra-utility-meta.sh; break; fi
...
##Gaming Apps
echo ""
echo "Would you like install gaming applications, such as Steam and related tools?"
set -- $(locale LC_MESSAGES)
yesexpr="$1"; noexpr="$2"; yesword="$3"; noword="$4"
while true; do
    read -p "(${yesword} / ${noword})? " yn
